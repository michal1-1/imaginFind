...
class SmartProcessingStrategy:
    """אסטרטגיית עיבוד חכמה"""

    def __init__(self):
        self.max_time_budget = 2 * 3600  # 2 שעות מקסימום
        self.target_completion_days = 3  # מטרה לסיים תוך 3 ימים
        self.min_batch_size = 10  # מינימום תמונות בכל ריצה
        self.max_batch_size = 300  # מקסימום תמונות בכל ריצה

    def calculate_optimal_batch_size(self, total_images, avg_processing_time=None):
        """חישוב גודל batch אופטימלי"""

        # חישוב בסיסי לפי מטרת הזמן
        daily_target = total_images / self.target_completion_days
        time_based_batch = min(daily_target, self.max_batch_size)

        # התאמה לפי זמן עיבוד ממוצע
    ...
    def get_system_performance_factor(self):
        """מחזיר מקדם ביצועים בהתבסס על מצב המערכת"""
        try:
